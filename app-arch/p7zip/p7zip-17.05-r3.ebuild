...
SRC_URI="https://github.com/p7zip-project/p7zip/archive/refs/tags/v17.05.tar.gz -> ${P}.tar.gz"
...
src_install() {
    ...
    # 🧩 Install SDK headers for system-lib consumers
    insinto /usr/include/p7zip
        while IFS= read -r -d '' hdr; do
           local subdir="${hdr#C/}"
           insinto "/usr/include/p7zip/C/$(dirname "${subdir}")"
           doins "${hdr}" || die "Failed to install ${hdr}"
		done < <(find C -type f -name '*.h' -print0)
}
